...
export class CommentRepository implements ICommentRepository {
    async create(comment: Comment): Promise<Comment> {
        try{
            ...
            if (result.insertId) {
                return new Comment(result.insertId, comment.postId, comment.authorId, comment.parentId, 0, [], comment.content, false, false, new Date(), new Date());
            }
    ...
    async getById(id: number): Promise<Comment> {
        try {
            ...
            const [rows] = await conn.data.execute<RowDataPacket[]>(
                'SELECT id, post_id, author_id, parent_id, content, is_deleted, is_flagged, created_at, updated_at FROM comments WHERE id = ?',
                [id]
            );
            if (rows.length > 0) {
                const r = rows[0];
                return new Comment(r.id, r.post_id, r.author_id, r.parent_id, r.content, r.is_deleted, r.is_flagged, r.created_at, r.updated_at);
            }
            return new Comment();
        } catch {
            return new Comment();
    ...
    async hasLiked(userId: number, commentId: number): Promise<boolean> {
        try {
            ...
            if (!conn.success || !conn.data) return false;
            const [rows] = await conn.data.execute<RowDataPacket[]>(
                'SELECT COUNT(*) as count FROM comment_likes WHERE user_id = ? AND comment_id = ?',
                [userId, commentId]
            );
            return rows[0].count > 0;
        } catch {
